...
use ostd::{
    mm::{
        DmaDirection, DmaStream, DmaStreamSlice, FrameAllocOptions, Infallible, VmReader, VmWriter,
        PAGE_SIZE,
    },
    sync::SpinLock,
    Pod,
...
pub fn basic_request_stateless<T: CryptoSessionTrait>(
    device: Arc<CryptoDevice>,
    opcode: T::DataOpcode,
    flf: &mut T::DataFlfStateless,
    vlf_in: &T::DataVlfStatelessIn,
    ...
    let request_info = session_request(&device.data_queue, &header, true, flf, vlf_in);
...
impl<'a, T: CryptoSessionTrait> CryptoSession<T> {
    pub fn new(
        ...
        let session_info = create_session(
            ...
            device
                .features
                .contains(FeatureBits::VIRTIO_CRYPTO_F_REVISION_1),
            flf,
            vlf,
        );
...
fn new_dma(
    len: usize,
    init: bool,
    mut func: impl FnMut(VmWriter<Infallible>),
...
fn request_by_bytes(
    ...
    in_dma: DmaStreamSlice<DmaStream>,
    out_bytes: &mut [u8],
) -> usize {
    ...
    out_dma
        ...
        .read(&mut VmWriter::from(out_bytes))
}

fn create_session<T: Pod>(
    control_queue: &SpinLock<VirtQueue>,
    header: &ControlHeader,
    stateless: bool,
    flf: &mut T,
    vlf: &impl VarLenFields<T>,
) -> CreateSessionInput {
...
fn destroy_session(
    ...
    let flf = DestroySessionFlf { session_id };
    let in_len = if stateless {
...
fn session_request<T: Pod, VlfOut: VarLenFields<T>>(
    data_queue: &SpinLock<VirtQueue>,
    header: &DataHeader,
    stateless: bool,
    flf: &mut T,
    vlf_in: &impl VarLenFields<T>,
) -> (VlfOut, CryptoInhdr) {
